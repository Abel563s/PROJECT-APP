<x-app-layout>
    <div class="py-4 px-4 space-y-4 max-w-[1700px] mx-auto font-inter">
        ...
        <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-4 pb-3 border-b border-slate-100">
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-[8px] font-black text-[#00ADC5] uppercase tracking-[0.3em] mt-1">Operational Reconnaissance Matrix</p>
                </div>
            </div>
            <div class="flex items-center gap-2">
                <div class="flex items-center gap-2 px-4 py-2 bg-white border border-slate-100 rounded-xl shadow-sm">
                    <div class="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse"></div>
                    <span class="text-[9px] font-black text-slate-900 tracking-widest uppercase leading-tight">{{ $updates->count() }} Reports Filed</span>
                </div>
        ...
        <!-- Search & Filter -->
        <form action="{{ route('admin.weekly-updates.index') }}" method="GET" class="space-y-4">
                ...
                <table class="w-full text-left">
                    <thead>
                        <tr class="bg-gradient-to-r from-[#00ADC5] to-[#00ADC5]">
                            <th class="px-6 py-3 text-[9px] font-black text-white uppercase tracking-widest border-b border-[#00ADC5]/30">Deployment</th>
                            <th class="px-6 py-3 text-[9px] font-black text-white uppercase tracking-widest border-b border-[#00ADC5]/30">Contact Person</th>
                            <th class="px-6 py-3 text-[9px] font-black text-white uppercase tracking-widest border-b border-[#00ADC5]/30">Responsible</th>
                            <th class="px-6 py-3 text-[9px] font-black text-white uppercase tracking-widest border-b border-[#00ADC5]/30">Expected Completion</th>
                            <th class="px-6 py-3 text-[9px] font-black text-white uppercase tracking-widest border-b border-[#00ADC5]/30">Timestamp</th>
                        ...
                        @forelse($updates as $update)
                            <tr class="group hover:bg-slate-50/80 transition-all duration-300 odd:bg-white even:bg-slate-50/20">
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-3">
                                        <div class="w-11 h-11 rounded-xl bg-white shadow border border-slate-100 flex items-center justify-center transition-all duration-300 group-hover:rotate-[10deg] group-hover:scale-110 group-hover:border-[#00ADC5]">
                                            <i data-lucide="file-text" class="w-5 h-5 text-slate-300 group-hover:text-[#00ADC5]"></i>
                                        </div>
                                        <div class="space-y-1">
                                            <p class="text-sm font-black text-slate-900 leading-tight group-hover:text-[#00ADC5] transition-colors">
                                                {{ $update->project->project_code }}
                                            </p>
                                ...
                                <td class="px-6 py-4">
                                    ...
                                    <p class="text-[9px] font-bold text-slate-400 uppercase tracking-widest">Client Contact</p>
                                </td>
                                <td class="px-6 py-4">
                                    <p class="text-xs font-black text-slate-900">{{ $update->responsible_person ?? 'N/A' }}</p>
                                    <p class="text-[9px] font-bold text-slate-400 uppercase tracking-widest">Project Owner</p>
                                </td>
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-1.5">
                                        <i data-lucide="calendar" class="w-3.5 h-3.5 text-[#00ADC5]"></i>
                                        <p class="text-xs font-black text-slate-700">{{ $update->expected_completion_date ? $update->expected_completion_date->format('M d, Y') : '---' }}</p>
                                    </div>
                                </td>
                                <td class="px-6 py-4">
                                    <span class="text-[10px] font-black text-slate-400 uppercase tracking-widest">{{ $update->created_at->format('W, Y') }} (Week)</span>
                                </td>
                                <td class="px-6 py-4 text-right">
                                    <div class="flex items-center justify-end gap-1.5">
                                        ...
                                        @if(auth()->user() && auth()->user()->isAdmin())
                                            <form action="{{ route('admin.weekly-updates.destroy', $update->id) }}" method="POST" class="inline"
                                                onsubmit="return confirm('Are you sure you want to delete this weekly update?');">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit"
                            ...
                            <tr>
                                <td colspan="6" class="px-6 py-16 text-center">
                                    <div class="flex flex-col items-center gap-3">
                                        ...
                                        <h3 class="text-base font-black text-slate-900">Empty Logs</h3>
                                        <p class="text-[9px] uppercase font-black tracking-widest text-slate-400">No weekly reconnaissance reports filed yet.</p>
                                    </div>
                                </td>
                            </tr>
                        @endforelse
        ...
        <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:p-0">
            ...
            <div class="relative bg-white rounded-2xl shadow-2xl max-w-3xl w-full max-h-[90vh] overflow-y-auto">
                <div class="sticky top-0 bg-white border-b border-slate-100 px-6 py-3 flex items-center justify-between rounded-t-2xl z-10">
                    <h3 class="text-base font-black text-slate-900">Weekly Report Details</h3>
    ...
    <div id="editModal" class="hidden fixed inset-0 z-50 overflow-y-auto">
        <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:p-0">
            ...
            <div class="relative bg-white rounded-2xl shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto">
                <div class="sticky top-0 bg-white border-b border-slate-100 px-6 py-3 flex items-center justify-between rounded-t-2xl z-10">
                    <h3 class="text-base font-black text-slate-900">Edit Weekly Report</h3>
    ...
    <div id="createModal" class="hidden fixed inset-0 z-50 overflow-y-auto">
        <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:p-0">
            ...
            <div class="relative bg-white rounded-2xl shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto">
                <div class="sticky top-0 bg-white border-b border-slate-100 px-6 py-3 flex items-center justify-between rounded-t-2xl z-10">
                    <h3 class="text-base font-black text-slate-900">New Weekly Report</h3>
    ...
    <script>
        function openCreateModal() {
            fetch(`/admin/weekly-updates/create?partial=1`)
        ...
        function openViewModal(id) {
            fetch(`/admin/weekly-updates/${id}?partial=1`)
                .then(response => response.text())
                .then(html => {
        ...
        function openEditModal(id) {
            fetch(`/admin/weekly-updates/${id}/edit?partial=1`)
                .then(response => response.text())
                .then(html => {
